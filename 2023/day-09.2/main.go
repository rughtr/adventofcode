...
func (s intList) buildCalculationStack(stack *Stack[intList]) {
...
func (s intList) extrapolateNextValue() int {
	...
	s.buildCalculationStack(&calculationStack)
...
func (s intList) extrapolatePreviousValue() int {
	...
	for calculationStack.Any() {
		...
		if ok {
			p1 = calculationStack.Pop()
			extrapolated := p1[0] - result
			p1 = append(intList{extrapolated}, p1...)
			calculationStack.Push(p1)
		}
	}
...
func main() {
	...
	for _, numbers := range sequences {
		extrapolated := numbers.extrapolatePreviousValue()
		sum += extrapolated
	}
